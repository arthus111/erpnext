...
login.bind_events = function () {
	...
	$(".toggle-password").click(function () {
		...
		if (input.attr("type") == "password") {
			...
			$(this).text('{{ _("Απόκρυψη") }}')
		} else {
			input.attr("type", "password");
			$(this).text('{{ _("Εμφάνιση") }}')
		}
	});
...
login.call = function (args, callback) {
	login.set_status('{{ _("Επιβεβαίωση...") }}', 'blue');
	...
	var login_handlers = {
		200: function (data) {
			if (data.message == 'Logged In') {
				login.set_status('{{ _("Επιτυχής") }}', 'green');
		...
		401: get_error_handler('{{ _("Αποτυχία Σύνδεσης. Ξαναπροσπαθήστε") }}'),
